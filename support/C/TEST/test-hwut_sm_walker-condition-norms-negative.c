 ...
 *
 * Same as 'test-hwut_sm_walker-condition-norms.c' only that the conditions
 * in the state machine are negated, i.e. they are 'not' conditions. 
 *
 ...
 * (1) OK tests:
 *
 *   -- Set all condition norms to 'true' and let all conditions respond
 *      with 'true'.
 *   -- Set all condition norms to 'false' and let all conditions respond
 *      with 'false'.
 *   -- Set all condition norms to 'any' and let all conditions respond
...
hwut_sm_transition_t TM_0[] = { { 0, -1, &states[1] } }; /* HERE difference. */
...
main(int argc, char** argv) 
{
	...
	hwut_info("State Machine Walker: Condition Norms (negative conditions);\n" 
              "CHOICES: 1, 2a, 2b, 2c, 2d, 2e, 2f;\n")
