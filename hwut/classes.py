...
class TestApplication:
    ...
    def request_information_if_necessary(self):
        ...
        if self.make_required_f and not self.make_successful_f: 
            if self.title == "": 
                self.title = "[no response to --hwut-info, no title]"
            if self.choice_list == []:
...
class TestApplicationDB:
    def __init__(self):
        ...
        self.__active_mode_f = False

    def set_active_mode_f(self, Value=True):
        """By default, HWUT does not interact with the filesystem to 
           get the list of files to consider. Only if it is in active
           mode, it searches for executable and makeable files.
        """
        self.__active_mode_f = Value
    ...
    def init(self, TestApplicationSelection=[]):
        ...
        if self.__active_mode_f == False: return
    ...
    def get_test_execution_sequence(self, SpecificApplicationList=[], FailedOnlyF=False, 
                                    SpecificChoice=""):
	# RETURNS:  [] if no application does not exits and cannot be made
	#           None if an application does not exist in the database
        ...
        if SpecificApplicationList == []: 
            ...
            for application in SpecificApplicationList:
                if not self.db.has_key(application):
		    return None
                else:
    ...
    def __sequentialize_sub_experiments(self, StartIdx, entry, FailedOnlyF, SpecificChoice):
        ...
        for choice in entry.choice_list:
            ...
            present_f = True
            if entry.make_required_f and not entry.make_successful_f: 
                present_f = False
